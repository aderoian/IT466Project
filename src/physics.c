...
void physics_detect_collision() {
    int i, j;
    PhysicsBody *a, *b;
    CollisionShape aWorld = {0}, bWorld = {0};
    ...
    for (i = 0; i < body_manager.count; i++) {
        ...
        for (j = i + 1; j < body_manager.count; j++) {
            ...
            gfc_vector3d_add(aWorld[0], a->position, a->shape[0]);
            gfc_vector3d_add(aWorld[1], a->position, a->shape[1]);
            gfc_vector3d_add(bWorld[0], b->position, b->shape[0]);
            gfc_vector3d_add(bWorld[1], b->position, b->shape[1]);
